...
const UpdateCategoryForm = ({ set_show_sidebar, get_all_categories, category }: any) => {
    ...
    useEffect(() => {
        if (category) {
            ...
            set_category_description(category?.category_description)
            set_created_at(moment(category?.created_at).format("DD/MM/YYYY"))
        }
    }, [category])

    const update_category = async (e: FormEvent) => {
        e.preventDefault()
        ...
        try {
            set_is_loading(true)
            const resp = await axios.post(`${baseUrl}/course-categories/update/${category?.category_id}`, formData, authorizationObj)
    ...
    return (
        <>
            ...
            <form onSubmit={update_category} className='w-full flex flex-col justify-start items-start gap-4'>
                <TextField
                ...
                />
                <TextField
                    label="Creation Date" fullWidth
                    variant="outlined" required value={created_at}
                />
                <Button
                    fullWidth type="submit" disabled={is_loading}
                    variant='contained' color="secondary"
                >
                    <RiPencilLine style={{ marginRight: "8px", marginTop: "-2px" }} /> Update Category
                </Button>
            </form>
        </>
    )
