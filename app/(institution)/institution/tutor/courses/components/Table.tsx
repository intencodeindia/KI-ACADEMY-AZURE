...
import { authorizationObj, baseUrl, courseThumbnailPath, courseVideoPath, lectureVideoPath } from "@/app/utils/core";
...
export const SingleLecture = ({ lecture, get_lectures, section_id, singleCourse, is_loading, set_is_loading, set_error_message, set_success_message }: any) => {
    ...
    const update_lecture = async () => {
        ...
        formData.append("file_type", media_type)
...
export const ViewSingleCourse = ({ singleCourse, setSingleCourse, getAllCourses }: any) => {
    ...
    const [prices, set_prices] = React.useState([])
                        ...
                        <CardContent sx={{ padding: 3 }}>
                            ...
                            <Grid container spacing={2} sx={{ marginTop: 0.5 }}>
                                ...
                                <Grid item xs={12} sm={6}>
                                    <TextField
                                        label="Language"
                                        value={capitalizeString(course_language) || ""}
                                        // InputProps={{ readOnly: true }}
                                        variant="outlined"
                                        fullWidth
                                        sx={{ marginBottom: 1 }}
                                        onChange={(e: any) => set_course_language(e.target.value)}
                                    />
                                </Grid>
                                <Grid item xs={12} sm={6}>
                                    <Autocomplete
...
const TTable = ({ data, getAllCourses }: any) => {
    ...
    const dynamicColumns = [...example_obj
        ...
        .map(item => ({
            field: item,
            headerName: item === "instructor_name" ? "Institute Name" : formatString(item),
            width: 150,
            renderCell: (params: any) => {
